...
CREATE TABLE visitor (
    ...
    state VARCHAR2(2),
    zipcode VARCHAR2(10) 
); --storing information associated with the visitor
...
INSERT INTO visitor (visitor_id, visitor_name, visitor_email, address, state, zipcode) 
VALUES (104, 'Lucas Grenst', 'lgrenst@umbc.edu', '555 College Ave', 'MD', '20740'); --total 5 visitors are inserted into the database
...
INSERT INTO tour VALUES (6, 120);  -- 2 hours
...
START WITH 105
INCREMENT BY 1; --start at 105 for the new sequence as already the default data entries are set till 104 to create new visitor IDs
...
as 
v_count int; --keeping track of the count of visitors to check for matching e-mails
v_vid int; -- a new variable ID placer to store new visitors
begin 
 select count(*) into v_count from visitor where visitor_email = v_email; --checks whether any visitor with the same email exists which that count will be sent to v_count
 ...
 if v_count > 0 then -- Based on that count, if it's greater than zero, message will be outputted that the visitor exists
  ...
  v_vid := visitor_seq.nextval; -- If the vistor's email doesn't exist, they are new entrants to the system  and a new ID is generated for them
  ...
  dbms_output.put_line('New visitor ID is ' || v_vid); -- The new vistor ID's is also outputted for confirmation
 end if; 
end;
/
...
v_tid transaction.transaction_id%type; --Transaction ID
v_ttype transaction.transaction_type%type; -- Transaction Type
v_fname facility.facility_name%type; --Facility Name
v_start transaction.start_time%type; -- Transaction Start Time
v_days transaction.number_of_days%type; -- Number of Days of active transaction
v_status transaction.status%type; -- Status of transaction
v_total transaction.total_price%type; -- Total Price of the Transaction
v_vid visitor.visitor_id%type; -- The ID of the visitor
...
begin
 ...
 dbms_output.put_line('Transactions for ' || v_name || ':'); --If visitor exists, system starts to print out the transactional information associated with the visitor
 
 open c1; --open the cursor loop to extract our the transactional info linked to the visitor 
 loop
  fetch c1 into v_tid, v_ttype, v_fname, v_start, v_days, v_status, v_total; --connect to the variables we want to be outputed 
  exit when c1%notfound; -- exit if the rows associated with the cursor isn't found
  dbms_output.put_line('Trans ID: ' || v_tid || 
                       ...
                       ' | Days: ' || v_days || 
                       ' | Status: ' || v_status || 
                       ' | Total: $' || v_total); --Properly formatted transactional information
 end loop;
 close c1; --end the loop and close the cursor to end its intended use
...
is
 v_count int; --keeping track of park's count to see if it actually exists or not in the first place
 v_conflict_count int; -- keeping count of conflicting reservation
 v_actual_count int := 0; -- keeping count of correct campsites and actual max num of people it could fit where the zero help supports the following loop as a placer
 
 v_start date; --since timestamps exist in out data tables, date is needed to convert that claim
 v_end date;
 ...
 v_fid facility.facility_id%type; --Facility ID
 v_fname facility.facility_name%type; -- Facility Full Name
 v_max campsite.max_people%type; --Maximum people that an campsite could fit in
 ...
 cursor c1 is 
  ...
  where f.facility_id = c.facility_id 
    ...
    and c.max_people >= v_people; --Proper condition set in place to extract out proper campsite people occupancy and see which is sufficient or not to fit user's requested people count number
...
begin
 ...
 if v_count = 0 then 
  dbms_output.put_line('No such park.');
  return; -- ends this statement if v_count is zero condition becomes true
 end if;
 ...
 where f.facility_id = c.facility_id 
   and f.park_id = p.park_id 
   and p.park_name = v_pname
   and c.max_people >= v_people; -- have the maximal number of people greater than or equal to the number of people in the input 

 if v_count = 0 then 
  dbms_output.put_line('No matches found'); -- If there is no such campsite, print 'No matches found' and stop if the user's people count is very big that the campsite can't fulfill
  return; 
 end if;
 ...
 open c1; -- activate cursor loop by opening it and analyzing all applicable campsites
 loop
  fetch c1 into v_fid, v_fname, v_max; --get all data entries from the facility id, facility name, and max num of people that an campsite could fill in
  exit when c1%notfound; --stop loop if condition becomes false by not returning any data entry
  ...
  where t.facility_id = v_fid
    ...
    and trunc(t.start_time) < v_end; --To check for duration overlap, less than is placed to see if the user's end date is preceding the already present transactiob
  ...
  if v_conflict_count = 0 then
   ...
   v_actual_count := v_actual_count + 1; -- Adds in to the initial placer that we set up to state how an vacant campsite was found
  end if;
  
 end loop;
 close c1; --End the cursor loop to prevent excessive memory usage
...
IS
    ...
    v_total_price NUMBER(10,2); -- NUMBER(10,2),  10 = total digits allowed, 2 = digits after decimal point
    v_facility_name VARCHAR2(100);
...
BEGIN
    ...
    v_total_price :=
        ...
        (NVL(v_child_price,0) * p_num_children); -- NVL replaces NULL with 0, so price calculation does not become NULL
...
set serveroutput on; --start the output to become visible in the script output
...
select * from visitor where visitor_email = 'jbenson88@gmail.com'; --show old entry of Jeff and their existing information in the database
...
select * from visitor where visitor_name = 'Suhan Roy'; -- shows the confirmation of Suhan's data entry
exec list_visitor_transactions('Suhan Roy'); --But, since no transactions are present, just the title mentioning Transactions for Suhan would be mentioned, and the below details would be empty as there are no transactional traces for Suhan in the data
...
VALUES (
    ...
    165,
    1
); --New transaction is created with an type 1 indicating that it is active, type 2 for campsite originating from the facility ID which is Great Falls Park having the ID number of 5. 
